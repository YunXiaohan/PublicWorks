...
# 최소 및 최대 강도 값을 하드코딩으로 설정
min_intensity = 0
max_intensity = 1023
exposure_time_ms = 0.15
...
class CameraApp:
    ...
    def setup_camera(self):
        ...
        self.camera.exposure_time_us = int(exposure_time_ms * 1000)
        self.camera.gain = 0
    ...
    def update_frame(self):
        ...
        if frame is not None:
            ...
            image_clipped = np.clip(self.image, min_intensity, max_intensity)
            image_8bit = ((image_clipped - min_intensity) / (max_intensity - min_intensity) * 255).astype(np.uint8)
    ...
    def on_closing(self):
        if self.camera:
            self.camera.disarm()
            self.camera.dispose()  # SDK의 문서에 따르면 close 대신 dispose를 사용할 수 있습니다.
        self.sdk.dispose()
        self.root.destroy()
